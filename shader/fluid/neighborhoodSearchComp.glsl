...
void main()
{
    ...
    ivec3 initCellPos = ivec3(floor(particle[pi].predict / cellSize));
    uint neighborNum = 0;
        ...
        for (y = -1; y <= 1; ++y)
        {
            for (z = -1; z <= 1; ++z)
            {
                ...
                for (j = s; j < e; ++j)
                {
                    ...
                    if (ni != pi)
                    {
                        vec3 d = particle[pi].predict - particle[ni].predict;
                        float dist2 = dot(d, d);
                        if (dist2 <= 1.1 * neighborRadius * neighborRadius)
                        {
                            const uint neighborIndex = pi * maxNeighbors + neighborNum;
                            neighbor[neighborIndex] = ni;
                            ++neighborNum;
                        }
                    } 
