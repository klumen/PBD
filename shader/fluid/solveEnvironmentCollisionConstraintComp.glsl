...
void collision(vec3 p, vec3 n)
{
    ...
    uint pi = i + head;
    float dis = SDF(particle[pi].predict, p, n);
    if (dis < 0.0)
    {
        dis = -dis;
        particle[pi].corr += dis * n;
        vec3 deltaPosition = particle[pi].predict + particle[pi].corr - particle[pi].position;
        vec3 deltaPosition_t = deltaPosition - dot(deltaPosition, n) * n;
        float deltaLengh = length(deltaPosition_t);
        if (deltaLengh < mu_s * dis)
        {
            particle[pi].corr -= deltaPosition_t;
        }
        else
        {
            particle[pi].corr -= deltaPosition_t * min(1.0, mu_k * dis / deltaLengh);
        }
    }
}
